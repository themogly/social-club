...
- Status: merged (owner-authorised) — VISUAL VERIFICATION PENDING (built blind, no browser)
- Verified each audit item vs live code first (palette/enum/border subset already landed @ 38c0e40, not re-done). Built: shared <x-button> (primary/secondary/danger/danger-soft/warning/outline × sm-xl, <a> on href, attribute passthrough, focus ring) adopted on socio CTAs (harmonises the socio↔counter drift) + render test; a11y (one h1 per counter screen via the shared top-bar, role/aria-live on flash [error→assertive alert] + offline banners, role=img+label on the footfall heatmap, placeholder contrast /60→full muted token across every counter input); tailored empty states on the 4 sibling member relation managers + socio history dashed-cards; behaviour-preserving refactors Batch::scopeDispensable() (FEFO + 3 POS stock queries route through it) + ApproveApplication→RecordMemberConsent; failed-jobs borders/semantic colours themed for dark mode. Counter-screen <x-button> adoption + per-screen descriptive h1 text = documented visual-pass follow-ups. No migration → MySQL parity N/A. 417 green.

## Prompt 35 — camera QR scanning: check-in + dispensary POS (built fresh)
- Status: merged (owner-authorised) — LIVE CAMERA DECODE PENDING (needs a device; no headless camera here)
- Built the shared camera scanner (the prompt-22/28 skip): a progressive enhancement over the wedge scanner + name search that never gates identification. Per-location opt-in via camera_scan_enabled (added to Settings::DEFAULTS false; excluded from the org settings page as per-location). [CORRECTION (prompt 59): this line originally claimed "the LocationForm toggle now drives it" — untrue at the time. The readers used Settings::get() (Setting rows) while the LocationForm toggle wrote the locations.settings JSON column, a disconnected store. Prompt 59 reconciled both to one store, so the toggle NOW genuinely drives it.] Shared component (components/counter/camera-scan.blade.php + the cameraScan Alpine behaviour in the previously-empty app.js) uses native BarcodeDetector where available and hides its trigger where not (manual inputs remain). getUserMedia rear camera; stream + loop torn down on close/decode/livewire:navigating. A decoded QR calls $wire.submitCameraScan(token) on both screens → sets $scan → existing submitScan() → ResolveMemberByToken (QR_CARD, sha256) — one lookup path, no id in payload. Wired into check-in + POS Identify, each @if(cameraScanEnabled). Reuses <x-button>. Tests: gate off/on both screens + token→member routing + unknown-token flash. Vite build verified. DEFERRED (documented): jsQR bundled fallback for Safari/Firefox/iPad (needs a browser to add+verify); until then those degrade to manual. No migration (setting only) → MySQL parity N/A. 421 green.

## Prompt 40 — audit log viewer: formatted, plain-language changes
- Status: merged (owner-authorised) — screenshots pending (no browser here)
- The audit entry viewer rendered raw JSON cents/cg + snake_case column names. Now two generic classes format it: AuditFieldFormatter (value → euros/grams/enum label/date/Yes-No, keyed off the *_cents/*_cg column SUFFIX not the Eloquent cast — Member::declared_monthly_cg is cast plain integer, so cast-first would print raw cg; a cast check remains secondary) + AuditFieldLabeler (field → its real Filament label, Table→Infolist→Form first match, reusing the resource's own translations; settings.updated null-model resolves from ManageSettings::form(); no-home fields → Str::headline() + a warning deduped per model+field). Label build uses the resource's List page as the owner (Schema::make(null) throws in getLivewire()); map from the panel's getResources()/getModel(). Raw before/after JSON demoted into a native collapsed <details> (still e()-escaped). Added @property-read auditable_type to AuditLog for Larastan. Tests: till-close cents→euros, declared_monthly_cg→grams matching MemberInfolist, enum→label, settings null-model keeps _eur/_g, collapsed raw view, generalises across TillSession/Member/Expense, unlabeled field logged once. No migration → MySQL parity N/A. 428 green.

## Prompt 41 — bar POS charge confirmation visible in place
- Status: merged (owner-authorised) — screenshots pending (no browser)
- Root cause (from the Livewire cycle, not template-guessing): state is fine (lastOrderId + flash survive commit; resetBasketState clears neither — Livewire test confirms); the issue is discoverability — the prominent flash rendered only in the page-top banner, off-screen when scrolled to the basket. Fix: render the same $flashMessage colocated in the basket column right above Cobrar (reused mechanism + tokens + role/aria-live, wire:key flash-basket), covering success and every error path. Manual-amount markup structurally sound (standard grid/token spacing) — no DOM overlap; reported artifact was likely the OS launcher per the prompt. Tests: success/error flash rendered colocated (2× in HTML) + lastOrderId retained + receipt link resolves. No migration → MySQL parity N/A. 430 green.

## Prompt 42 — counter screen switcher
- Status: merged (owner-authorised) — screenshots pending (no browser)
- Added a screen switcher to the shared counter top-bar (one component, all four screens get it). Inline icon+label pills (labels from lg, icon-only below, overflow-x-auto for narrow) — tablet-first, no dropdown. Permission-filtered per the real per-screen gate (checkin.manage / pos.use / pos.bar / till.open||till.close) so no 403-in-waiting; active screen marked non-link via routeIs()+aria-current; unsaved-work confirm reused verbatim from the Panel link. data-counter-screen/data-counter-switch markers for tests. Tests: full-access sees 4 (current active), pos.bar-only sees only Barra, confirm guard on switch links, active marked per screen. New labels bilingual. No migration → MySQL parity N/A. 434 green.

## Prompt 43 — bar sales itemised reporting + oversight
- Status: merged (owner-authorised) — screenshots pending (no browser)
- Added the missing itemised layer under FinancialReport's existing "Barra" aggregate (untouched). OrderResource (read-only, canCreate false, Table+Infolist, filters status/location/operator/date, voided-order void audit shown; OrderPolicy gained viewAny pos.bar||reports.view; FORMLESS-documented). OrdersRelationManager on Member (withoutGlobalScope Location, org-wide purchase history; added Member::orders()). BarSalesReport + page (informes/ventas-barra) via ReportPage/AbstractReport — top articles by units+revenue, aggregating order items JSON in PHP; manual lines grouped by name so the grand total reconciles to the cent with FinancialReport's Barra (tested). Tests (8): list scoping denial, viewAny denial, voided-order void audit, member-tab isolation, report aggregation/reconciliation/exclusion. Resource scaffolding built via a subagent (Larastan-verified after). No migration → MySQL parity N/A. 442 green.

## Prompt 44 — settings coverage audit
- Status: merged (owner-authorised) — screenshots pending (no browser)
- Continues prompts 24/34. Fixed the pos_* dead-toggle bug via OPTION A (per-location): DispensaryPos now reads restrict_pos_to_checked_in / signature_on_dispensation (the LocationForm toggle keys, per active location) instead of the org-wide pos_require_checked_in / pos_signature_required, which were retired — one setting per behaviour, read + editable; tested end-to-end (toggling flips enforcement). Wired aforo_default as a new Location's capacity default (was zero-consumer). Added default_locale + enabled_locales (SCALARS STRING+JSON) + minute_quorum_fraction_pct (edge→bp) to ManageSettings — all already read by ResolveLocale/CreateMinute; UI writes the same keys (tested: ResolveLocale flips, 60%→6000bp). forecast_options_g + consent_text_version deferred (documented). Completeness test updated. No migration → MySQL parity N/A. 447 green.

## Prompt 46 — membership fee collection at the till
- Status: merged (owner-authorised, batch) — screenshots pending (no browser)
- RecordFeePayment had zero callers → unpaid_fee (BLOCK at counter) permanently unsatisfied → every member with a fee was blocked from dispensing. Built the collection path: a "Cobrar cuota" till action (mirrors recordExpense) gated on the NEW membership.fee.collect permission (separate from fee.override), reusing DispensaryPos's member search, collecting CASH (→ TillSummary.fees_cash) or WALLET (FEE ledger movement, debt-limit respected) via RecordFeePayment against the same membership feesPaid() checks; partial/instalment with remaining shown. Members-section: enrol/renew still only set fee_cents (regression-tested), now show a prompt-to-till notification (+ "Ir a la caja" link) + a "Cuota pendiente" badge column. Retroactive: DOCUMENTED (staff collect existing fees via the till) — no synthetic backfill. Tests (8). No migration → MySQL parity N/A. 455 green.

## Prompt 59 — one per-location settings store
- Status: merged (owner-authorised, batch) — screenshots + MySQL parity pending (no browser; MySQL won't start here)
- Root cause behind prompt 44: two disconnected per-location stores — Setting rows (what Settings::get reads) vs a locations.settings JSON column (what LocationForm toggles wrote, read only by the never-called AutoSettleDebt). Kept the Setting-row store, RETIRED the JSON column. Added Settings::get(key, default, ?locationId) for explicit-location reads; LocationForm's 5 toggles (SETTING_TOGGLES const) are now virtual fields loaded/persisted as location-scoped Setting rows by Edit/CreateLocation; AutoSettleDebt::isRingFenced reads Settings::get(ring_fenced, false, location). Migration copies JSON→rows then drops the column (ring_fenced survives). bar_enabled given a real consumer (BarPos refuses when off + switcher hides the Barra link; default true). Corrected the camera_scan_enabled claim in BUILD-LOG + appended a note to prompt 44's DECISIONS. Tests: override overrides at A not B; no-override falls to org not DEFAULTS; EDIT-FORM drives the POS end-to-end (the path prompt-44's test masked); ring_fenced survives as a row; JSON store gone; GENERIC dead-key guard over SETTING_TOGGLES. Migration → MySQL parity pending. 461 green.

## Prompt 48 — audit trail completeness
- Status: merged (owner-authorised, batch) — screenshots pending (no browser)
- Wired RecordAuditLog into the five gaps prompt 17 named. Rule: audit in the domain action where one exists, else Filament afterSave; INSIDE the existing DB transaction (matches CommitStockTake — a failed audit rolls back the movement). wallet.adjusted (RecordWalletTransaction ADJUSTMENT, balance before/after); stock.adjusted/stock.merma (RecordStockMovement, remaining before/after + the merma reason); batch.intake (IntakeBatch, now transactional). Resource edits via a shared AuditsResourceChanges trait (snapshot raw original in beforeSave, diff saved values in afterSave — Filament fills after beforeSave; credential keys excluded): article.updated, genetic.updated, member.updated. user.roles.updated diffs the roles relation. Used .updated not the prompt's .price.updated (accurate — whole-record edits); flagged genetic PRICES have no edit surface (prompt 54). Corrected AUDIT-FINDINGS AD4. Append-only test still passes. Tests (7). No migration → MySQL parity N/A. 468 green.

## Prompt 60 — Charge always produces an observable outcome
- Status: pushed, NOT merged (per prompt) — screenshots pending (no browser)
- Root cause (code analysis; no real browser here): the client-side silent-disabled swallow — both POS Charge buttons bound x-bind:disabled="! online || @js($commitDisabled)", and a native disabled <button> eats its wire:click with no feedback/no livewire/update. requireOperator ruled out (it flashes + opens the PIN panel). Couldn't reproduce the headless "enabled, no update" capture in a real browser, but the fix closes the class either way. Decision: disable ONLY for offline (x-bind:disabled="! online") — offline's banner is driven by the same `online`, so disabled ⟺ reason-shown; every other blocked state stays clickable → commit() flashes its reason (guards unchanged/none loosened). Applied to bar AND dispensary; dispensary gained a colocated flash beside its button (mirrors prompt 41). Tests (11). No migration → MySQL parity N/A. 479 green.
