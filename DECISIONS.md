...
## Prompt 233 — the snapshot harnesses were measuring in the runner's OS font

Every `file://` snapshot harness (225's column, 230's both-bars, 231's wizard fit) inlines the built CSS —
which carries **root-relative** `url(/build/assets/inter-*.woff2)`. A `file://` page has no document root, so
every Inter face failed and the browser silently fell back to whatever the machine had. Measured here, same
commit, same build:

| | `document.fonts.check('16px Inter')` | latin face | a probe string at 16px |
|---|---|---|---|
| snapshot (`file://`) | **false** | **error** | **322px** |
| live page (`http://`) | true | loaded | **339px** |

**5% narrower text**, on every text-driven number those harnesses assert. On a Mac the fallback is
Helvetica-ish and 231's wizard "fits"; on Linux it is DejaVu-ish and the same harness reports *"content 467,
region 464 — FAIL, 3px over"*. **Neither machine was measuring the app.** The wizard itself was never wrong:
454px in Inter, both locales, at every asserted size.

This is the audit-the-instruments family, and it was a CLASS — three harnesses, one cause.

### `await document.fonts.ready` is not the check

Worth its own line, because it is the obvious thing to write and it does not work: `document.fonts.status`
was already `"loaded"` on the broken snapshot while the latin face sat in `error`. The status reports that
the loading PROCESS finished, not that anything succeeded. `document.fonts.check()` answers the question that
matters — can the browser use this family — and the helper also counts faces in `error`.

### The fix: the snapshot loads the app's own woff2

`InlinesBuiltCss` rewrites the stylesheet's root-relative asset URLs to absolute `file://` paths into
`public/build`. **Chosen over inlining the subsets as `data:` URLs** because it is exact (the very file the
app serves, not a copy), adds nothing to the snapshot's size, and cannot drift from the build. These
snapshots are regenerated by their harness test on every run, so an absolute local path inside one is not a
portability problem — it is what makes the capture faithful.

### The guard: every geometry harness proves what it measured in

`tests/Browser/font-ready.mjs` — `assertRealFont(page, label, fail)`, called by all three before their first
assertion, failing with **"MEASURING IN A FALLBACK FONT"** and the face counts. Proven by pointing a run at a
snapshot with its font URLs put back to root-relative:

```
FAIL: step1/es/1180x820/light: MEASURING IN A FALLBACK FONT — Inter is not usable
      (0/28 faces loaded, 4 in error). Every text-driven number from this run is the
      runner's OS font, not the app's.
```

A future harness written without the helper is still caught the first time it runs on a box whose fallback
does not happen to fit — but the helper is the cheap way to catch it on the first run instead.

### The corrected numbers, measured in Inter

```
step1/es/1180x820   content=454 region=454 panel=689 headroom=132 overflow=0 mrzInView=true
step1/es/1180x760   content=454 region=454 panel=689 headroom=72  overflow=0 mrzInView=true
step1/en/1180x820   content=454 region=454 panel=689 headroom=132 overflow=0 mrzInView=true
step1/en/1180x760   content=454 region=454 panel=689 headroom=72  overflow=0 mrzInView=true

working/1180x820    rows 6@52px · nag 298×98, text 272px/1 line, controls 128×44 120×44
bar/1180x820        rows 4@58px          blocked/1180x820  amber 5.82:1
bar-grid  both bars 58px / 58px          bar-list  both bars 52px / 52px
```

**And the two instruments now agree.** Live page, same box, same step: `content=454 region=454 panel=689
overflow=0` at 1180×820 **and** 1180×760 — identical to the snapshot. That agreement is the point of the
branch; before it, the snapshot and the app disagreed by a font.

**Honesty about what changed here:** on THIS machine the macOS fallback and Inter happen to give step 1 the
same 454px, so 231's recorded figure was right by luck. The proof that the instrument was broken is the probe
(322 vs 339px), the `error` face state, and the Linux run — not a changed step-1 number. What did move is
smaller and telling: 228's photo-nag controls measure **128×44 / 120×44** in Inter against 127×44 / 119×44 in
the fallback. A 1px text-width difference is exactly the size of number these harnesses were inventing.

### Verification

`composer check` green — **1833 tests**, 1830 passed, 3 pre-existing skips, Larastan 0, Pint clean. **MySQL
was left to CI**, per the running order.

**Test instruments only.** Nothing under `app/`, `resources/`, `routes/` or `lang/` changed — 231's rhythm and
cap are exactly as it left them, because the app was never wrong. The three harnesses keep every assertion and
